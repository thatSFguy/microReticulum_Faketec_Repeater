...
- `-DBAKED_LXMF_PRESENCE_ENABLE` Announce this node under the LXMF `lxmf.delivery` aspect with a display name in `app_data`, so LXMF clients like [MeshChat](https://github.com/liamcottle/reticulum-meshchat), Sideband and NomadNet can show it in their network visualizer. Independent of the telemetry announce — same transport identity, different aspect, different cadence. The `app_data` wire format is `msgpack.packb([display_name_bytes, stamp_cost])` matching the reference implementation in [micropython-reticulum](https://github.com/attermann/micropython-reticulum) `firmware/urns/lxmf.py`. Without this flag, the node still routes traffic but is invisible to LXMF peer visualizers.
- `-DBAKED_LXMF_DISPLAY_NAME='"<name>"'` The UTF-8 display name this node announces itself as. Keep it short (≤ ~32 bytes). Example: `-DBAKED_LXMF_DISPLAY_NAME='"Solar Site A"'`. Default `"Faketec Repeater"`. Requires `BAKED_LXMF_PRESENCE_ENABLE`. **The outer single quotes and inner double quotes are both required** because the value is embedded as a C string literal at compile time.
- `-DBAKED_LXMF_INTERVAL_MS=<ms>` Interval between LXMF presence announces in milliseconds. Default `1800000` (30 minutes). Short enough to show up reliably in client visualizers, long enough not to saturate a LoRa channel. Requires `BAKED_LXMF_PRESENCE_ENABLE`.
- `-DBAKED_LXMF_FIRST_MS=<ms>` Delay before the very first LXMF announce on boot, in milliseconds. Default `15000` (15 s). Requires `BAKED_LXMF_PRESENCE_ENABLE`.
